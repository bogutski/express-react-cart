      ...
      bcrypt.hash(req.body.password, 10, (bcryptError, hash) => {
        if (bcryptError) {
          return res.status(500)
            .json(message.error(bcryptError));
        }
        ...
        user
          ...
          .then((result) => {
            res.status(201)
              .json(message.success('User created'));
          })
          .catch((userError) => {
            res.status(500)
              .json(message.error(userError));
          });
      });
    })
    .catch((error) => {
...
export const userLogin = (req, res, next) => {
  User.find({ email: req.body.email })
    ...
    .then((user) => {
      if (!user.length) {
        res.status(401)
          .json(message.error('Auth failed. Email'));
      }
      ...
      bcrypt.compare(req.body.password, user[0].password, (err, result) => {
        ...
        res.status(401)
          .json(message.error('Auth failed'));
      });
    })
    .catch((err) => {
      res.status(500)
  ...
  User.findById(id)
    ...
    .then((doc) => {
      if (doc) {
        ...
        res.status(404)
          .json(message.error('No user for provided id'));
      }
    })
    .catch((err) => {
      res.status(500)
...
export const userUpdateById = (req, res, next) => {
  ...
  User.update({ _id: id }, { $set: req.body })
    ...
    .then((doc) => {
      if (doc.n) {
        res.status(200)
          .json(message.success('User updated'));
      } else {
        res.status(400)
